...
const USNATL_URL =
  'https://raw.githubusercontent.com/frishberg/Archive-of-US-News-College-Rankings/HEAD/data.csv'
...
async function fetchQsEditions() {
  ...
  for (const [year, nid] of Object.entries(QS_EDITIONS)) {
    ...
    while (page <= 20) {
      const r = await fetch(
        `https://www.topuniversities.com/rankings/endpoint?nid=${nid}&page=${page}&items_per_page=500&tab=indicators`,
        {
          headers: {
            'User-Agent': UA,
            Referer: 'https://www.topuniversities.com/world-university-rankings',
      ...
      for (const n of nodes) {
        ...
        if (rank != null) e.ranks[year] = rank
      }
...
async function fetchUsNews() {
  ...
  do {
    const r = await fetch(
      `https://www.usnews.com/education/best-global-universities/api/search?format=json&page=${page}`,
      {
        headers: {
          'User-Agent': UA,
          Accept: 'application/json',
    ...
    if (!r.ok) {
      if (page === 1) throw new Error('USN ' + r.status)
      break
    }
    const d = await r.json()
    totalPages = d.total_pages
    for (const it of d.items || []) {
      const rk = (it.ranks || []).find((x) => /best global/i.test(x.label))
      ...
      if (rank != null)
        items.push({
          ...
          url: it.url,
          rank,
          blurb: clean(it.blurb),
        })
    }
    process.stdout.write(`\rU.S. News: ${page}/${totalPages} (${items.length})`)
    page++
...
async function fetchThe() {
  ...
  for (const yr of THE_YEARS) {
    ...
    try {
      d = await (
        await fetch(
          `https://www.timeshighereducation.com/json/ranking_tables/world_university_rankings/${yr}`,
          { headers: { 'User-Agent': UA, Accept: 'application/json' } },
        )
      ).json()
    } catch {
      process.stdout.write(`\rTHE ${yr}: skip`)
      continue
    }
    for (const it of d.data || []) {
      const rank = parseRank(it.rank)
      ...
      if (!e) {
        e = {
          name: clean(it.name),
          country: clean(it.location),
          url: it.url ? `https://www.timeshighereducation.com${it.url}` : '',
          ranks: {},
        }
        byKey.set(key, e)
      }
    ...
    process.stdout.write(`\rTHE: ${yr} (${byKey.size} unis)`)
    await new Promise((x) => setTimeout(x, 200))
  }
  process.stdout.write('\n')
  return byKey
...
async function fetchUsNatl() {
  ...
  for (const l of lines.slice(1)) {
    ...
    const ranks = {}
    years.forEach((y, i) => {
      const v = parseInt(c[i + 1], 10)
      if (!Number.isNaN(v)) ranks[y] = v
    })
    if (name && Object.keys(ranks).length) byKey.set(normKey(name), { name, ranks })
  }
  console.log(
    `U.S. News National: ${byKey.size} U.S. universities (${Math.min(...years)}–${Math.max(...years)})`,
  )
  return byKey
}
...
async function main() {
  ...
  for (const { info } of qsBySlug.values())
    if (info.country && info.region) regionByCountry.set(info.country, info.region)
  ...
  for (const [slug, e] of qsBySlug) {
    ...
    const the = theByKey.get(key)
    const natl = natlByKey.get(key)
    if (usn) {
      usedUsn.add(key)
      matched2++
    }
    ...
    universities.push({
      ...
      region: info.region,
      city: info.city || (usn ? usn.city : ''),
      description: usn ? usn.blurb : '',
      logo: info.logo,
      qsUrl: `https://www.topuniversities.com${info.path}`,
      ...(usn ? { usnewsUrl: usn.url } : {}),
      ...(the && the.url ? { theUrl: the.url } : {}),
  ...
  for (const u of usnItems) {
    ...
    universities.push({
      ...
      usnewsUrl: u.url,
      ...(the && the.url ? { theUrl: the.url } : {}),
      rankings: makeRankings(null, u, the, natl),
    })
  }
  ...
  for (const [key, the] of theByKey) {
    ...
    if (natl) usedNatl.add(key)
    universities.push({
      id: `the-${slugOf(the.url) || key.replace(/\s+/g, '-')}`,
      name: the.name,
      country: the.country,
      region: regionByCountry.get(the.country) || '',
      city: '',
      theUrl: the.url || undefined,
  ...
  const best = (u) =>
    Math.min(
      u.rankings.qs[FEATURED] ?? 9e9,
      u.rankings.usnews[FEATURED] ?? 9e9,
      u.rankings.the[FEATURED] ?? 9e9,
      u.rankings.usnatl[FEATURED] ?? 9e9,
    )
  universities.sort((a, b) => best(a) - best(b))
  for (const u of universities) if (!u.description) delete u.description

  const qsYears = Object.keys(QS_EDITIONS)
    .map(Number)
    .sort((a, b) => a - b)
  const meta = {
    years: META_YEARS,
    featuredYear: FEATURED,
    systems: ['qs', 'usnews', 'the', 'usnatl'],
    systemLabels: {
  ...
  await writeFile(
    path.join(DIR, 'universities.json'),
    JSON.stringify({ meta, universities }) + '\n',
  )
  ...
  const uniRows = universities.map((u) => ({
    ...
    qs_url: u.qsUrl || '',
    usnews_url: u.usnewsUrl || '',
    the_url: u.theUrl || '',
  }))
  await writeFile(
    path.join(DIR, 'universities.csv'),
    toCsv(uniRows, [
      'id',
      'name',
      'country',
  ...
  for (const u of universities)
    for (const sys of ['qs', 'usnews', 'the', 'usnatl'])
      for (const yr of META_YEARS) {
        const rk = u.rankings[sys][yr]
        if (rk != null) rankRows.push({ university_id: u.id, system: sys, year: yr, rank: rk })
      }
  await writeFile(
    path.join(DIR, 'rankings.csv'),
    toCsv(rankRows, ['university_id', 'system', 'year', 'rank']),
  )

  const datapackage = {
    name: 'world-university-rankings',
    title: 'World University Rankings — QS, U.S. News & Times Higher Education',
    ...
    sources: Object.entries(meta.sources).map(([k, v]) => ({
      title: meta.systemLabels[k],
      path: v,
    })),
    resources: [
      {
        ...
        schema: {
          ...
          fields: [
            ...
            { name: 'city', type: 'string', title: 'Location' },
            { name: 'description', type: 'string' },
            { name: 'qs_url', type: 'string' },
            { name: 'usnews_url', type: 'string' },
            { name: 'the_url', type: 'string' },
          ],
        },
      },
      {
        ...
        schema: {
          foreignKeys: [
            { fields: 'university_id', reference: { resource: 'universities', fields: 'id' } },
          ],
          fields: [
            { name: 'university_id', type: 'string' },
            {
              name: 'system',
              type: 'string',
              constraints: { enum: ['qs', 'usnews', 'the', 'usnatl'] },
            },
            { name: 'year', type: 'integer' },
            { name: 'rank', type: 'integer', description: 'World rank (lower is better)' },
          ],
        },
      },
    ],
...
`
  ...
  const withMulti = (sys) =>
    universities.filter((u) => META_YEARS.filter((y) => u.rankings[sys][y] != null).length >= 2)
      .length
  console.log(
