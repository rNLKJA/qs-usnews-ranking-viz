  ...
  return (
    <aside className="flex h-full w-72 shrink-0 flex-col border-r border-border bg-sidebar text-sidebar-foreground">
      <div className="flex items-center gap-3 p-5">
        <div
          className="flex size-11 items-center justify-center border border-border bg-white p-2"
          style={{ borderRadius: '22%' }}
        >
          <img
            src="/brand-logo.svg"
            alt="Ranking Radar logo"
            className="size-full object-contain [filter:invert(1)]"
          />
        </div>
      ...
      <div className="space-y-4 p-4">
          ...
          {ALL_SYSTEMS.map((s) => {
            ...
            return (
              <button
                ...
                style={
                  on
                    ? { borderColor: SYSTEM_COLOR[s], color: SYSTEM_COLOR[s], borderWidth: 2 }
                    : undefined
                }
              >
                <span
        ...
        <div className="space-y-2">
          ...
          <p className="text-[9px] uppercase tracking-widest text-muted-foreground">
            QS 2016–27 · THE 2011–26 · USN-Global 26 · USN-National (US) 1984–2025
          </p>
        </div>
            ...
            <ScrollArea className="min-h-0 flex-1 px-2 pb-2">
              <ul>
                {list.slice(0, MAX_LIST).map((u) => (
                  <li key={u.id}>
                    <button
                      onClick={() => onToggleFocus(u.id)}
                      title="Add / remove from comparison"
                      aria-pressed={focusedIds.includes(u.id)}
                      className={`flex w-full items-center gap-3 px-2 py-2 text-left transition-colors duration-200 hover:bg-accent ${focusedIds.includes(u.id) ? 'bg-accent' : ''}`}
                    >
                      <span className="relative flex h-7 w-7 shrink-0 items-center justify-center">
                        ...
                        {focusedIds.includes(u.id) && (
                          <span
                            ...
                            style={{ background: focusColor(focusedIds, u.id) }}
                          />
                        )}
                      </span>
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="flex flex-wrap gap-x-2 text-[10px] uppercase tracking-widest">
                          {ALL_SYSTEMS.map((s) =>
                            u.rankings[s][y] != null ? (
                              <span key={s} style={{ color: SYSTEM_COLOR[s] }}>
                                {SYSTEM_SHORT[s]} #{u.rankings[s][y]}
                              </span>
                            ) : null,
                          )}
                        </span>
      ...
      <div className="grid grid-cols-3 gap-2 p-3">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center justify-center gap-1.5 border border-border px-2 py-2 text-[10px] uppercase tracking-widest transition-colors duration-200 hover:bg-foreground hover:text-background"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center justify-center gap-1.5 border border-[#ff3c3c] px-2 py-2 text-[10px] uppercase tracking-widest text-[#ff3c3c] transition-colors duration-200 hover:bg-[#ff3c3c] hover:text-white"
        >
