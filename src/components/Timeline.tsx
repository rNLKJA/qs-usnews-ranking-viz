...
export default function Timeline({
  ...
  const { laid, domain, chartWidth } = useMemo(() => {
    ...
    if (focused) {
      ...
      const pad =
        minRank === maxRank
          ? Math.max(1, Math.round(minRank * 0.15))
          : Math.max(1, Math.round((maxRank - minRank) * 0.3))
      return {
        laid,
        domain: [Math.max(1, minRank - pad), maxRank + pad] as [number, number],
        chartWidth: containerW,
      }
    }
    return {
      laid,
      domain: [1, maxRank] as [number, number],
      chartWidth: Math.max(MIN_WIDTH, MARGIN.left + maxRank * pxPerRank + MARGIN.right),
    }
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setRevealRank(
        Math.max(1, Math.ceil(x.invert(el.scrollLeft + el.clientWidth + REVEAL_BUFFER))),
      )
    }
    update()
  ...
  const connectors = useMemo(() => {
    ...
    return focusedIds
      .map((id) => {
        ...
        const pts = active
          .map((s) => {
            const r = uni.rankings[s][y]
            return r != null ? `${x(r)},${laneY(s)}` : null
          })
          .filter(Boolean)
        return pts.length > 1
          ? { id, color: focusColor(focusedIds, id), points: pts.join(' ') }
          : null
      })
      .filter((c): c is { id: string; color: string; points: string } => !!c)
  }, [focused, focusedIds.join(), universities, year, x, active.join()])

  const totalShown = active.reduce(
    (n, s) => n + laid[s].filter((p) => focused || p.rank <= revealRank).length,
    0,
  )
  ...
  return (
    <div className="relative overflow-hidden border border-border bg-card">
      ...
      <div
        ref={scrollRef}
        className={focused ? 'overflow-hidden' : 'overflow-x-auto overflow-y-hidden'}
      >
                ...
                return (
                  ...
                  >
                    <HoverCard openDelay={100} closeDelay={80}>
                      ...
                      <HoverCardContent
                        side={active.indexOf(system) === active.length - 1 ? 'top' : 'bottom'}
          ...
          {isEmpty && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 text-center">
              <p className="text-sm font-medium text-foreground">
                {focused
                  ? `No ${year} ranking for the selected schools`
                  : `No ranking data for ${year}`}
              </p>
              <p className="max-w-xs text-xs text-muted-foreground">
                Drag the year to one with data.
              </p>
            </div>
          )}
        </div>
